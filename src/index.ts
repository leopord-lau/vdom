...
const vdom = parse(document.querySelectorAll('body')[0].innerHTML);
document.body.appendChild(vdom._render())

const button = document.createElement('button');
button.innerText = "add new li";
button.addEventListener('click', addLi);
document.body.appendChild(button);


const vul = parse(document.querySelectorAll('ul')[0].outerHTML);

function addLi() {
  console.log('hi')
  const li = document.createElement('li');
  li.innerText = "newly add";
